...
public class OrderTaxTest {
  ...
  public void init() {
    ...
    // Online and US tax are $0 and Europe is $3.40
  ...
  public void testTaxOnlineOrder() {
    ...
    assertEquals(0.0, order.getTax(), TOLERANCE);
  ...
  public void testTaxEuropeOrder() {
    Order order = new Order("Europe-Order", Location.EUROPE);
    order.processCart(cart1);
    assertEquals(3.40, order.getTax(),TOLERANCE);
    order.processCart(cart2);
    assertEquals(25.40, order.getTax(),TOLERANCE);
  }
  
  @Test
  public void testTaxUSAOrder() {
    Order order = new Order("USA-Order", Location.USA);
    order.processCart(cart1);
    assertEquals(0,order.getTax(),TOLERANCE);
    order.processCart(cart2);
    assertEquals(10.00, order.getTax(), TOLERANCE);
